...
(function( $ ) {
  ...
  var OPTIONS = {
    ...
    settings: {
      ...
      value: "5;7",
      dimension: "",
      leftInfPos: {bound: -0.5, value: -1},
      rightInfPos: {bound: 100.5, value: 101}
    },
  ...
  jSlider.prototype = {
    ...
    limits: function( x, pointer ) {
			...
			// base limit
			if (x < 0) {
				if (this.settings.infOnLeft == true &&
					(x < this.settings.leftInfPos.bound)) {
					x = this.settings.leftInfPos.value;
				} else {
					x = 0;
				}
			}
			...
			if (x > 100) {
				if (this.settings.infOnRight == true &&
					(this.settings.rightInfPos.bound < x)) {
					 x = this.settings.rightInfPos.value;
				} else {
					x = 100;
				}
			}
		...
		redraw: function( pointer ) {
			...
			var htmlVal = "";
			if (this.settings.infOnRight == true && pointer.value.origin == Number.POSITIVE_INFINITY) {
				htmlVal = '> ' + this.nice(this.settings.to);
				pointer.ptr.addClass("infinity");
				pointer.isInfinity = true;
			} else if (this.settings.infOnLeft == true && pointer.value.origin == Number.NEGATIVE_INFINITY) {
				htmlVal = '< ' + this.nice(this.settings.from);
				pointer.ptr.addClass("infinity");
				pointer.isInfinity = true;
			} else {
				htmlVal = this.nice(pointer.value.origin);
				if (pointer.isInfinity == true) {
					pointer.isInfinity = false;
					pointer.ptr.removeClass("infinity");
				}
			}
		...
		redrawLabels: function( pointer ) {
			...
			if( !this.settings.single ){
				...
				var prcAnother = another.value.prc;
				prcAnother = prcAnother > 100 ? 100 : prcAnother;
				prcAnother = prcAnother < 0 ? 0 : prcAnother;
				var sizesAnother = {
					label: another_label.o.outerWidth(),
					right: false,
					border: ( prcAnother * this.sizes.domWidth ) / 100
				};
				
				setPosition(label, sizes, prc);
				setPosition(another_label, sizesAnother, prcAnother);
				
				switch( pointer.uid ){
					case 0:
						if(label.o.offset().left + label.o.outerWidth() > another_label.o.offset().left) {
							var ndash = "";
							var prefix = "";
							var pointerHtml = "";
							var anotherHtml = "";
							if (another.isInfinity == true) {
								pointerHtml = this.nice( pointer.value.origin );
								prefix = "> ";
							} else if (pointer.isInfinity == true) {
								anotherHtml = this.nice( another.value.origin );
								prefix = "< ";
								var tmp = another_label;
								another_label = label;
								label = tmp;
							} else {
								anotherHtml = this.nice( another.value.origin );
								pointerHtml = this.nice( pointer.value.origin );
								another_label.value.html(anotherHtml);
								ndash = "&nbsp;&ndash;&nbsp;";
							}
							...
							if( another.value.prc != pointer.value.prc ){
								label.value.html(prefix + pointerHtml + ndash + anotherHtml);
					...
					case 1:
						if (label.o.offset().left < another_label.o.offset().left + another_label.o.outerWidth() + 0.5){
							var ndash = "";
							var prefix = "";
							var pointerHtml = "";
							var anotherHtml = "";
							...
							if( another.value.prc != pointer.value.prc ){
								label.value.html(prefix + anotherHtml + ndash + pointerHtml );
								sizes.label = label.o.outerWidth();
								sizes.border = ( prc * this.sizes.domWidth ) / 100;
							}
						} else {
    ...
    prcToValue: function( prc ){
			...
			if (this.settings.infOnRight == true && prc == this.settings.rightInfPos.value) {
				return Number.POSITIVE_INFINITY;
			}
			if (this.settings.infOnLeft == true && prc == this.settings.leftInfPos.value) {
  	...
  	valueToPrc: function( value, pointer ) {
  		if (this.settings.infOnRight == true && value == Number.POSITIVE_INFINITY) {
				return this.settings.rightInfPos.value;
			}
  		if (this.settings.infOnLeft == true && value == Number.NEGATIVE_INFINITY) {
				return this.settings.leftInfPos.value;
			}  		
